...
def load_models():
    ...
    distilbert_model = pipeline("question-answering", model="distilbert-base-cased-distilled-squad")
    embedder = SentenceTransformer('paraphrase-MiniLM-L6-v2')  # For answer re-ranking
    return bert_model, roberta_model, distilbert_model, embedder

# Load all models at startup
bert_model, roberta_model, distilbert_model, embedder = load_models()
...
def ensemble_answer(question, context):
    ...
    distilbert_answer = distilbert_model(question=question, context=context)
    
    # Combine answers
    answers = [bert_answer, roberta_answer, distilbert_answer]
...
if uploaded_file is not None:
    ...
    if st.button("Get Answer"):
        if question and document_content:
            ...
            st.write(f"Confidence Score: {validated_answer['score']*100:.4f}")
        else:
